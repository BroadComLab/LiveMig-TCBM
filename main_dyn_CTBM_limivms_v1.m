...
% ----- COSTANTS
TOU_UPDATE  =   1;  % { 0 = koushner  --> verificato
                    %   1 = our vers 2
                    %   2 = fast update k/n }
PLOT_FIG    =   1;
...
% -------------------------------------------------------------------
% ----- VARIABLES --------------------------------------------------
Imax        =   6;              % Final round index
N0          =   1*10^4;         % Iteration index [] 
...
M0          =   64;            % size of memory [Mega bit] 
Esetup      =   5.9;            %5.9; % per il wifi %% energy wasted by connection setim phase Joule 
gamma       =   1000;            % buono tra 100 e 1000
amax        =   2*10^(-3);         %2*10^(-4);%5*10^(-7);% buono tra 5*10(-7) e 10^(-6)
passoTou    =   100;
DeltaTM     =   94.6;%21.33;%2*10^600;2*10^300;       %0.17*10^(1); % maximum tolerated Time Migration
DeltaDT     =   0.1; %1.17*10^-3;%19.31;%10^600;         %11.63636364; % maximum tolerated down-time
beta        =   1;%5.17;            %1.05; %1.2; %beta_i(1);
...
maxExpVal   =   10^300;         % Limita gli esponenziali nei gradienti dei lamda.
...
else% INIZIALIZZO FITTIZIAMENTE A 2
...
% ------ BEGIN ROUND CYCLE ---------------------------------------------
for n = 1:N0
    % SE NOT-FEASIBLE esci subito!
    if feasibility==0 
        break;
    end

    % --------- CALCOLO SOMMATORIE PARZIALI 
    % 
        %maxExpVal = 10^12;
    sum1_Imax = zeros(Imax+2,1);
    sum2_Imax = zeros(Imax+2,1);%(0+1)*((Wavg^0)/exp(Rtilde(n)))^0;
    sum3_Imax = 0;%((Wavg^0)*exp(Rtilde(n)))^0;
    expSumRi1 = zeros(Imax+2,1);

    % -------- CALCOLO exp Sum R1
    for i = 2:Imax+2 % esponenziale calcolato per i = 0 - Ima+1
        expSumRi1(i) = min(exp((alfa_Pw-1)*Rtilde(n,i)-sum(Rtilde(n, 1:i-1))),maxExpVal); %ok
    end
    
    % ------- CALCOLO Sum 1 e Sum 2
    for i = 1:Imax+1 %
        for k=i+1:Imax+2 %ok
            sum1_Imax(i) = sum1_Imax(i) + M0*K0*C(k)*min(exp((alfa_Pw-1)*Rtilde(n,k) - sum(Rtilde(n,1:k-1))),maxExpVal);
        end
        for k=i:Imax+2
            sum2_Imax(i) = sum2_Imax(i) + C(k)*min(exp(-(sum(Rtilde(n,1:k)))),maxExpVal); %OK
        end
    end
    
    for k=0:Imax+1
        sum3_Imax = sum3_Imax + C(k+1)*min(exp(-(sum(Rtilde(n,1:k+1)))),maxExpVal); % corretta k+1
    end
    
    % --------- FINE CALCOLO SOMMATORIE PARZIALI
    
    % -------- TCBM Calcolo sommarorie e Ttm Tdt 
    sumTdt1 = 0;
    for k = 1:Q-1
        sumTdt1 = sumTdt1 + Rtilde(n, k*S+1);
    end
    sumTmt1 = 0;
    for k = 0:Q-1
        % -- 
        for l = k*S+1:(k+1)*S
            sumTmt2 = 0;
            for p = 1:k-1
                sumTmt2 = sumTmt2 + Rtilde(p*S+1);
            end
            sumTmt1 = sumTmt1+ Wavg^l + (delta_f(k) * exp(-l*Rtilde(n, 1)) + 1-delta_f(k)*exp((k*S-l)*Etilde(k*S-l)-S *(sumTmt2)));
        end
    end
    
    Tdt =  M0*exp(-Rtilde(n, 0))* ...+
        ( delta_f(1+Imax)+(1-delta_f(1+Imax))*Wavg^(1+Imax)*exp(-Rtilde(n, Imax+1)-S*Rtilde(n, 1)-(1-delta_f(Q-1))*S(sumTdt1)) );% da init zeros(1,Imax+2);
    Ttm =  M0*exp(-Rtilde(n, 0))*(1+Wavg^(1+Imax) * exp(-Rtilde(n, Imax+1)-S*Rtilde(n, 1)-(1-delta_f(Q-1))*S(sumTdt1)) + ...+
        (1-delta_f(Imax)) * (sumTmt1));% da init zeros(1,Imax+2);
    
    % --- Fine TCBM Codice TM DT
    
    baseV = 1;
    % ------ TVBM calcolo dei LAMDA
    
    gradL1temp = teta*((Ttm / DeltaTM)-1); 
    gradL2temp = ((Tdt/DeltaDT)-1);
    gradL3temp = zeros(1,Imax+1);
    gradL3temp(1,1) = teta*(beta*Wavg*exp(-Rtilde(n,1))-1);
    gradL3temp(1,2) = teta*(beta*Wavg*exp(-Rtilde(n,2))-1);
    
    for i=1:Q-1
        gradL3temp(1,i*S+1) = teta*((beta*Wavg*min(exp(-Rtilde(n,i*S+1)), maxExpVal))-1);%*round(1000*((beta*Wavg*exp(-Rtilde(n))-1)))/1000);
    end
    
     % ------ calcolo gradienti L1 L2 L3
%     gradL1temp = teta*((M0/DeltaTM)*round((100000*(sum3_Imax))/100000)-1); 
%     gradL2temp = (((M0/DeltaDT)*C(Imax+2)*min(exp(-sum(Rtilde(n,:))),maxExpVal))-1);%(round(1000*((exp(-Rtilde(n))*(M0/DeltaDT)*(Wavg/exp(Rtilde(n)))^(Imax+1))-1))/1000);
% 
%     gradL3temp = zeros(1,Imax+1);
%     for i=1:Imax+1
%         gradL3temp(i) = teta*((beta*Wavg*min(exp(-Rtilde(n,i)), maxExpVal))-1);%*round(1000*((beta*Wavg*exp(-Rtilde(n))-1)))/1000);
%     end
...
%     gradRtilde(n,:) = max(min( round(gradRtildetemp.*fatRound)./fatRound , ones(1,Imax+2)*limiteGradienti), -ones(1,Imax+2)*limiteGradienti);
    ...
    sumDerGrad1 = 0;
    for k = 1:Q-1
        sumDerGrad1 = sumDerGrad1 + Rtilde(n, k*S+1);
    end
    sumDerGrad2 = 0;
    for m=0:Q-1
        for p=1:m-1
            sumInDerGrad1 = sumInDerGrad1+ Rtilde(n, p*S+1);
        end
        for l=m*S+1:(m+1)*S
           sumDerGrad2 = sumDerGrad2 + W^l*(delta_f(m)*exp((alfa_Pw-l)*Rtilde(n, 1))+1-delta_f(m)*exp((alfa_Pw+mS-l)*Rtilde(n, m*S+1)-S*Rtilde(n, 1)-(1-delta_f(m-1))*(sumInDerGrad1)) ); 
        end
    end
    
    derGrad_R0 = (alfa_Pw-1)*K0*M0*exp((alfa_Pw-1)*Rtilde(n, 0))- teta*K0*M0*exp(-Rtilde(n, 0)) * ...+
        (Wavg^(1+Imax)*exp((alfa_Pw-1)*Rtilde(n, Imax+1)-S*Rtilde(n, 1)- (1-delta_f(Q-1))*S*(sumDerGrad1)) + ...+
        (1-delta_f(Imax))*( sumDerGrad2 ));
    
    gradRtildetemp(0) = derGrad_R0 - teta*lamda_1*(1/DeltaTM)*Ttm - lamda_2*(1/DeltaDT)*Tdt - teta*lamda_3(0)*beta*Wavg*exp(-Rtilde(n, 0));
    ...
    for  k=0:Q-1
        for l=(k*S+1):(k+1)*S
            sumDerGradR1a = sumDerGradR1a + Wavg^l*(delta_f(k)*l)*exp( -l*Rtilde(n, 1) ) + ...+
                (1-delta_f(k))*S*exp( (k*S-l)*Rtilde(n, k*S+1)-S*Rtilde(n, 1)-(1-delta_f(k-1))*S * sum_f(Rtilde(n,:), 1, k-1, S) );
            
            sumDerGradR1b = sumDerGradR1b + Wavg^l*((l-alfa_Pw)*delta_f(k))*exp((alfa_Pw-l)*Rtilde(n, 1) ) + ...+
                (1-delta_f(k))*S*exp( (alfa_Pw+k*S-l)*Rtilde(n, k*S+1)-S*Rtilde(n, 1)-(1-delta_f(k-1)) * sum_f(Rtilde(n, :), 1, k-1, S) );
        end
    end
    
    derGrad_R1_Ttm = -M0*exp(-Rtilde(n, 0))*(S*Wavg^(1+Imax))*exp(-Rtilde(n, Imax+1)-S*Rtilde(n, 1)-(1-delta_f(Q-1))*S*sum_f(Rtilde(n, :), 1, Q-1, S)) + ...+
        (1+delta_f(Imax))*( sumDerGradR1a );
    
    derGrad_R1_Tdt = -S*M0*exp(-Rtilde(n, 0))*Wavg^(1+Imax)*(1-delta_f(1+Imax))*exp(-Rtilde(n, Imax+1)-S*Rtilde(n, 1)-(1-delta_f(Q-1))*S*sum_f(Rtilde(n, :), 1, Q-1, S));
    
    derGrad_R1_en = -teta*K0*M0*exp(-Rtilde(n, 0))*(S*W^(1+Imax)*exp((alfa_Pw-1)*Rtilde(n, Imax+1)-S*Rtilde(n, 1)-(1-delta_f(Q-1))*S*(sum_f(Rtilde(n,:) ,1,Q-1,S))) + ...+
        (1-delta_f(Imax))*(sumDerGradR1b));
    
    gradRtildetemp(1) = derGrad_R1_en + lamda_1*teta/(DeltaTM)*derGrad_R1_Ttm + lamda_2/DeltaDT * derGrad_R1_Tdt - lamda_3(1)*beta*Wavg*exp(-Rtilde(n, 1));
    ...
    for j=1:Q-1 % ???
        ...
        for  k=j:Q-1
            for l=(k*S+1):(k+1)*S
                sumDerGradRi_a = sumDerGradRi_a + Wavg^l*(delta_f(k-j)*(j*S-l))*exp( (j*S-l)*Rtilde(n, j*S+1) -S*Rtilde(n, 1)-(1-delta_f(k-1))*S*(sum_f(Rtilde(n,:),1,j-1,S))) - ...+
                    (1-delta_f(k-j))*S*(1-delta_f(k-1))*exp( (k*S-l)*Rtilde(n, k*S+1)-S*Rtilde(n, 1)-(1-delta_f(k-1))*S * sum_f(Rtilde(n, :), 1, k-1, S) );

                sumDerGradRi_b = sumDerGradRi_b + Wavg^l*(1-delta_f(k))*(delta_f(k-j)*(alfa_Pw+j*S-l)*exp((alfa_Pw+j*S-l)*Rtilde(n, j*S+1) -S*Rtilde(n, 1)-(1-delta_f(k-1))*sum_f(Rtilde(n, :),1,j-1,S)) - ...+
                    (1-delta_f(k-j))*(1-delta_f(k-1))*exp( (alfa_Pw+k*S-l)*Rtilde(n, k*S+1)-S*Rtilde(n, 1)-(1-delta_f(k-1)) * sum_f(Rtilde(n, :), 1, k-1, S) ));
            end
        end
        
        derGrad_Ri_Tdt = -S*(1-delta_f(Q-1))*M0*exp(-Rtilde(n, 0))*Wavg^(1+Imax)*(1-delta_f(1+Imax))*exp(-Rtilde(n, Imax+1)-S*Rtilde(n, 1)-(1-delta_f(Q-1))*S*sum_f(Rtilde(n, :), 1, Q-1, S));

        derGrad_Ri_Ttm = M0*exp(-Rtilde(n, 0))*(-Wavg^(1+Imax))*S*(1-delta_f(Q-1))*exp(-Rtilde(n, Imax+1)-S*Rtilde(n, 1)-(1-delta_f(Q-1))*S*sum_f(Rtilde(n, :), 1, Q-1, S)) + ...+
            (1+delta_f(Imax))*( sumDerGradRi_a );

        derGrad_Ri_en = teta*K0*M0*exp(-Rtilde(n, 0))*(-(1-delta_f(Q-1))*S*exp((alfa_Pw-1)*Rtilde(n, Imax+1)-S*Rtilde(n, 1)-(1-delta_f(Q-1))*S*(sum_f(Rtilde(n, :),1,Q-1,S))) + ...+
            (1-delta_f(Imax))*(sumDerGradRi_b));
        
        gradRtildetemp(j*S+1) = derGrad_Ri_en + lamda_1/DeltaTM *derGrad_Ri_Ttm + lamda_2/DeltaDT * derGrad_Ri_Tdt - lamda_3(j*S+1)*beta*Wavg*exp(-Rtilde(n, j*S+1)) ;
        
    end
    
    % -------- R Imax+1
    
    derGrad_RImax_Ttm = -M0*(Wavg^(1+Imax))*exp(-Rtilde(0))*exp(-Rtilde(Imax+1)-S*Rtilde(1)-(1-delta_f(Q-1))*S*sum_f(Rtilde, 1, Q-1, S));
    derGrad_RImax_Tdt = -M0*exp(-Rtilde(0))*Wavg^(1+Imax)*(1-delta_f(1+Imax))*exp(-Rtilde(Imax+1)-S*Rtilde(1)-(1-delta_f(Q-1))*S*sum_f(Rtilde, 1, Q-1, S));
    derGrad_RImax_en = teta*K0*M0*W^(1+Imax)*exp(-Rtilde(0))*(alfa_Pw-1)*exp((alfa_Pw-1)*Rtilde(Imax+1)-S*Rtilde(1)-(1-delta_f(Q-1))*S*(sum_f(Rtilde,1,Q-1,S)));
    
    gradRtildetemp(Imax+1) = derGrad_RImax_en + lamda_1*teta/(DeltaTM)*derGrad_RImax_Ttm + lamda_2/DeltaDT * derGrad_RImax_Tdt;
...
%     % ------ CALCOLO GRadR per 0
%     gradRtildetemp(1) = (K0.*M0.*C(1).*(alfa_Pw-1).*min(exp((alfa_Pw-1)*Rtilde(1)),maxExpVal) - (teta.*round(1000.*((sum1_Imax(1))))./1000)) - ...+
%         lamda_1(n+1)*teta*(M0/DeltaTM)*(sum2_Imax(1)) - ...+
%         lamda_2(n+1)*(M0/DeltaDT)*C(Imax+2)*min(exp(-sum(Rtilde(n,:))),maxExpVal) - ...+
%         (lamda_3(n+1,1).*teta.*beta.*Wavg.*min(exp(-Rtilde(n,1)),maxExpVal));
%     if(teta~=0)
%     % ------ CALCOLO Gradiente Rtilde 1 - Imax
%     gradRtildetemp(2:Imax+1) = teta.*(K0.*M0.*C(2:Imax+1).*(alfa_Pw-1).*expSumRi1(2:Imax+1) - (round(1000.*((sum1_Imax(2:Imax+1))))./1000)) - ...+
%         lamda_1(n+1)*teta*(M0/DeltaTM)*(sum2_Imax(2:Imax+1)) - ...+
%         lamda_2(n+1)*(M0/DeltaDT)*C(Imax+2)*min(exp(-sum(Rtilde(n,:))),maxExpVal) - ...+
%         (lamda_3(n+1,2:Imax+1).*teta.*beta.*Wavg.*min(exp(-Rtilde(n,2:Imax+1)),maxExpVal))';
% 
%     % ------ CALCOLO GRadR per Imax+1 
%     gradRtildetemp(Imax+2) = teta*(K0.*M0.*C(Imax+2).*(alfa_Pw-1).*expSumRi1(Imax+2)) - ...+
%         lamda_1(n+1)*teta*(M0/DeltaTM)*C(Imax+2)*min(exp(-sum(Rtilde(n,:))),maxExpVal) - ...+
%         lamda_2(n+1)*(M0/DeltaDT)*C(Imax+2)*min(exp(-sum(Rtilde(n,:))),maxExpVal);
%     end
...
%     gradL3(n,:) = max(min( round(gradL3temp.*fatRound)./fatRound , ones(1,Imax+1).*limiteGradienti), -ones(1,Imax+1).*limiteGradienti);
    ...
    % CALCOLO valore Rtilde i+1th
    Rtilde(n+1,:) = min((Rtilde(n,:) - omega_i(n,:).*gradRtilde(n,:)),(ones(1,Imax+2).*log(Rcap)));
    ...
    sumEi = 0;
    for i=2:Imax+2
        sumEi = sumEi + K0*M0*C(i)*(R_i(n,i)^(alfa_Pw-1))*prod(R_i(1:i-1).^(-1));
    end
...
Tm_i(1) = M0/Ropt(1);
for i=2:Imax+1
    Vm_i(i) = Wavg*Tm_i(i-1);
    Tm_i(i) = Vm_i(i)/Ropt(i);
end
