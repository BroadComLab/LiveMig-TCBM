...
N0          =   5*10^7;         % Iteration index [] 
Wavg        =   5;              % dirty bit rate Mbit/sec
...
amax        =   3*10^(-4);      %2*10^(-4);%5*10^(-7);% buono tra 5*10(-7) e 10^(-6)
passoTou    =   100;
DeltaTM     =   2*10^300;%94.6;%21.33;%2*10^600;2*10^300;       %0.17*10^(1); % maximum tolerated Time Migration
DeltaDT     =   11.636364;%10^300;%0.1; %1.17*10^-3;%19.31;%10^600;         %11.63636364; % maximum tolerated down-time
beta        =   0;%5.17;            %1.05; %1.2; %beta_i(1);
...
Rtilde(1,:) = log(Wavg);%(Rmax+Wavg)/2);    %%%%%%%% START POINT %%%%%%%%%
...
% ---- #### NUOVA TCBM #### -----------------------
    ...
    for k = 0:Q-1
        ...
        for l = (k*S+1):((k+1)*S)
            ...
            sumTmt1 = sumTmt1+ Wavg^l + (delta_f(k) * exp(-l*Rtilde(n, bV+1)) + (1-delta_f(k))*exp((k*S-l)*Rtilde(n, bV+k*S+1)-S*Rtilde(n, bV+1)-(1-delta_f(k-1))*S*(sumTmt2)));
        end
    end
    
    Tdt =  M0*exp(-Rtilde(n, bV+0))* ...+
        ( delta_f(1+Imax)+(1-delta_f(1+Imax))*Wavg^(1+Imax)*exp(-Rtilde(n, bV+Imax+1)-S*Rtilde(n, bV+1)-(1-delta_f(Q-1))*S*(sumTdt1)) );% da init zeros(1,Imax+2);
    Ttm =  M0*exp(-Rtilde(n, bV+0))*(1+Wavg^(1+Imax) * exp(-Rtilde(n, bV+Imax+1)-S*Rtilde(n, bV+1)-(1-delta_f(Q-1))*S*(sumTdt1)) + ...+
    ...
    gradL3temp(1,bV+0) = teta*(beta*Wavg*exp(-Rtilde(n,bV+0))-1);
    gradL3temp(1,bV+(1:S)) = teta*(beta*Wavg*exp(-Rtilde(n,bV+1))-1);
...
if(Imax>=0)  %~Imax<0)
 if(Imax~=0)
    ...
    gradRtildetemp(bV+(1:S)) = derGrad_R1_en + lamda_1(n)*teta/(DeltaTM)*derGrad_R1_Ttm + lamda_2(n)/DeltaDT * derGrad_R1_Tdt - lamda_3(n, bV+ 1)*beta*Wavg*exp(-Rtilde(n, bV+1));
    ...
    for j=1:Q-1 % ???
        ...
        derGrad_Ri_en = teta*K0*M0*exp(-Rtilde(n, bV+0))*(-(1-delta_f(Q-1))*S*exp((alfa_Pw-1)*Rtilde(n, bV+Imax+1)-S*Rtilde(n, bV+1)-(1-delta_f(Q-1))*S*(sum_f(Rtilde(n, :),1,Q-1,S))) + ...+
            (1-delta_f(Imax))*(sumDerGradRi_b));
        %GRADIENTE j
        gradRtildetemp(bV+((j*S+1):((j+1)*S))) = derGrad_Ri_en + lamda_1(n)/DeltaTM *derGrad_Ri_Ttm + lamda_2(n)/DeltaDT * derGrad_Ri_Tdt - lamda_3(n, bV+j*S+1)*beta*Wavg*exp(-Rtilde(n, bV+j*S+1)) ;
    end
 end% Imax~=0
    ...
    gradRtildetemp(bV+Imax+1) = derGrad_RImax_en + lamda_1(n)*teta/(DeltaTM)*derGrad_RImax_Ttm + lamda_2(n)/DeltaDT * derGrad_RImax_Tdt;
end % Imax>0
    % ----------------------------------------
    % ------------- END NUOVI TCBM GRAD------
...
if ENE_SELECTNEW == 0
    sumEi = 0;
    for i=2:Imax+2
        sumEi = sumEi + K0*M0*C(i)*(R_i(n,i)^(alfa_Pw-1))*prod(R_i(n, 1:i-1).^(-1));
    end
...
if ENE_SELECTNEW == 1 
    ...
    for m=0:Q-1
        ...
        for l=(m*S+1):((m+1)*S)
            sumEneTC2 = sumEneTC2 + Wavg^l * (delta_f(m)*R_i(n, bV+1)^(alfa_Pw-l) + (1-delta_f(m))*(prod(R_i(n, bV+(0:m-1)).^(-S)))*R_i(n, bV+m*S+1)^(alfa_Pw+m*S+l)) ;
        end
    end
