...
amax        =   8*10^(-4);      %2*10^(-4);%5*10^(-7);% buono tra 5*10(-7) e 10^(-6)
passoTou    =   100;
...
if(Imax>=0)  %~Imax<0)
 if(Imax~=0)
    ...
    for j=1:Q-1 % ???
        ...
        %GRADIENTE j
        gradRtildetemp(bV+((j*S+1):((j+1)*S))) = derGrad_Ri_en + lamda_1(n)/DeltaTM *teta*derGrad_Ri_Ttm + lamda_2(n)/DeltaDT * derGrad_Ri_Tdt - lamda_3(n, bV+j*S+1)*beta*Wavg*exp(-Rtilde(n, bV+j*S+1)) ;
    end
 end% Imax~=0
