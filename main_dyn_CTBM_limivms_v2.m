...
% -------------------------------------------------------------------
% ----- VARIABLES --------------------------------------------------
Imax        =   2;              % Final round index
N0          =   8*10^5;         % Iteration index [] 
...
Esetup      =   3*10^-4;%;            %5.9; % per il wifi %% energy wasted by connection setim phase Joule 
gamma       =   1000;           % buono tra 100 e 1000
amax        =   0.1*10^(-3);      %2*10^(-4);%5*10^(-7);% buono tra 5*10(-7) e 10^(-6)
passoTou    =   100;
DeltaTM     =   117;%141;%2*10^300;%94.6;%21.33;%2*10^600;2*10^300;       %0.17*10^(1); % maximum tolerated Time Migration
DeltaDT     =   0.06;%1.1*10^-4;%10^300;%0.1; %1.17*10^-3;%19.31;%10^600;         %11.63636364; % maximum tolerated down-time
